# API Endpoints and Testing Practice Resources

## Example API Endpoints

- `GET https://google.com/resource/user/?username='xyz'`
- `GET https://samples.openweathermap.org/data/2.5/weather?q=London,uk&appid=2b1fd2d7f77ccf1b7de9b441571b39b8`
- [HTTPBin: HTTP Methods](https://httpbin.org/#/HTTP_Methods/get_get)

## Free Websites to Practice Selenium TestNG

Here are some free websites where you can practice Selenium TestNG without registration or payment:

- [Sauce Demo Site for Testing](https://www.saucedemo.com/)
- [The Internet Heroku App](https://the-internet.herokuapp.com/)
- [OrangeHRM Demo Site for Practice](https://opensource-demo.orangehrmlive.com/)
- [OnlyTesting Blog Demo Website](https://only-testing-blog.blogspot.com/)
- [TestPages Heroku App Demo Site](https://testpages.herokuapp.com/styled/index.html)
- [jQuery UI Demo Website for Selenium WebDriver Testing](https://jqueryui.com/demos/)
- [Omayo Blog for Online Testing](http://omayo.blogspot.com/)

These sites offer a variety of web elements and scenarios for testing, making them ideal for improving your automation skills. Please use these resources responsibly.

---

## Additional Learning Resources

**Step 1:** [FASTEST Way to Learn Coding and ACTUALLY Get a Job (YouTube)](https://www.youtube.com/watch?v=79pKwdiqcwI)

**Step 2:** [LearnPython.org](https://learnpython.org/) — FASTEST Way to Learn Coding and ACTUALLY Get a Job

**Step 3:** [12 Beginner Python Projects - Coding Course (YouTube)](https://www.youtube.com/watch?v=8ext9G7xspg)

**Step 5:** [Leetcode](https://leetcode.com/)

---

## MySQL Shell Commands

- [MySQL Shell Commands Tutorial (YouTube)](https://www.youtube.com/watch?v=4lBDiKIpeh4)
